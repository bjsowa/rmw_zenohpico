...
rmw_guard_condition_t *rmw_create_guard_condition(rmw_context_t *context) {
  ...
  guard_condition->implementation_identifier = rmw_zp_identifier;
  guard_condition->context = context;

  guard_condition->data =
      allocator->zero_allocate(1, sizeof(rmw_zp_guard_condition_t), allocator->state);
  ...
  if (rmw_zp_guard_condition_init(guard_condition_data) != RMW_RET_OK) {
...
rmw_ret_t rmw_destroy_guard_condition(rmw_guard_condition_t *guard_condition) {
  ...
  if (guard_condition->data) {
    rmw_zp_guard_condition_t *guard_condition_data = guard_condition->data;
    rmw_zp_guard_condition_fini(guard_condition_data);
    allocator->deallocate(guard_condition->data, allocator->state);
  }
